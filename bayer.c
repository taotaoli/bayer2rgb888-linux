...
void
ClearBorders(uint8_t *rgb, int img_width, int img_height, int w)
{
    int i, j;
    // black edges are added with a width w:
    i = 3 * img_width * w - 1;
    j = 3 * img_width * img_height - 1;
    while (i >= 0) {
        rgb[i--] = 0;
        rgb[j--] = 0;
    }

    int low = img_width * (w - 1) * 3 - 1 + w * 3;
    i = low + img_width * (img_height - w * 2 + 1) * 3;
    while (i > low) {
        j = 6 * w;
        while (j > 0) {
            rgb[i--] = 0;
...
void
ClearBorders_uint16(uint16_t * rgb, int img_width, int img_height, int w)
{
    int i, j;
...
dc1394error_t
dc1394_bayer_NearestNeighbor(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
    ...
    /* add black border */
    imax = img_width * img_height * 3;
    for (i = img_width * (img_height - 1) * 3; i < imax; i++) {
        rgb[i] = 0;
    }
    iinc = (img_width - 1) * 3;
    for (i = (img_width - 1) * 3; i < imax; i += iinc) {
        rgb[i++] = 0;
        rgb[i++] = 0;
        rgb[i++] = 0;
    }
...
dc1394error_t
dc1394_bayer_Bilinear(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
    ...
    ClearBorders(rgb, img_width, img_height, 1);
    rgb += rgbStep + 3 + 1;
    height -= 2;
    width -= 2;
...
dc1394error_t
dc1394_bayer_HQLinear(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
    ...
    ClearBorders(rgb, img_width, img_height, 2);
    rgb += 2 * rgbStep + 6 + 1;
    height -= 4;
    width -= 4;
...
dc1394error_t
dc1394_bayer_EdgeSense(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
...
/* coriander's Bayer decoding */
dc1394error_t
dc1394_bayer_Downsample(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    ...
    case DC1394_COLOR_FILTER_GRBG:        //---------------------------------------------------------
    case DC1394_COLOR_FILTER_GBRG:
        for (i = 0; i < img_height*img_width; i += (img_width<<1)) {
            for (j = 0; j < img_width; j += 2) {
                tmp = ((bayer[i + j] + bayer[i + img_width + j + 1]) >> 1);
                CLIP(tmp, outG[((i >> 2) + (j >> 1)) * 3]);
                tmp = bayer[i + img_width + j + 1];
                CLIP(tmp, outR[((i >> 2) + (j >> 1)) * 3]);
                tmp = bayer[i + img_width + j];
    ...
    case DC1394_COLOR_FILTER_RGGB:
        for (i = 0; i < img_height*img_width; i += (img_width<<1)) {
            for (j = 0; j < img_width; j += 2) {
                tmp = ((bayer[i + img_width + j] + bayer[i + j + 1]) >> 1);
                CLIP(tmp, outG[((i >> 2) + (j >> 1)) * 3]);
                tmp = bayer[i + img_width + j + 1];
                CLIP(tmp, outR[((i >> 2) + (j >> 1)) * 3]);
                tmp = bayer[i + j];
...
dc1394error_t
dc1394_bayer_Raw_Origin(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    uint8_t *outR, *outG, *outB;
    register int i, j;
    int tmp;
    ...
    for (i = 0; i < img_height; i++) {
        for (j = 0; j < img_width; j++) {
            tmp = bayer[i * img_width + j];
            CLIP(tmp, outG[(i * img_width + j) * 3]);
            CLIP(tmp, outR[(i * img_width + j) * 3]);
            CLIP(tmp, outB[(i * img_width + j) * 3]);
        }
    }
...
dc1394error_t
dc1394_bayer_Simple(const uint8_t *restrict bayer, uint8_t *restrict rgb, int img_width, int img_height, int tile)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
...
/* insprired by OpenCV's Bayer decoding */
dc1394error_t
dc1394_bayer_NearestNeighbor_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
...
/* OpenCV's Bayer decoding */
dc1394error_t
dc1394_bayer_Bilinear_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
...
/* High-Quality Linear Interpolation For Demosaicing Of
   ...
   Ross Cutler, in ICASSP'04 */
dc1394error_t
dc1394_bayer_HQLinear_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    const int bayerStep = img_width;
    const int rgbStep = 3 * img_width;
    int width = img_width;
    ...
    ClearBorders_uint16(rgb, img_width, img_height, 2);
    rgb += 2 * rgbStep + 6 + 1;
    height -= 4;
    width -= 4;
...
/* coriander's Bayer decoding */
dc1394error_t
dc1394_bayer_EdgeSense_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    /* Removed due to patent concerns */
    return DC1394_FUNCTION_NOT_SUPPORTED;
}
...
/* coriander's Bayer decoding */
dc1394error_t
dc1394_bayer_Downsample_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    ...
    case DC1394_COLOR_FILTER_GBRG:
        for (i = 0; i < img_height*img_width; i += (img_width<<1)) {
            for (j = 0; j < img_width; j += 2) {
                tmp =
                    ((bayer[i + j] + bayer[i + img_width + j + 1]) >> 1);
    ...
    case DC1394_COLOR_FILTER_RGGB:
        for (i = 0; i < img_height*img_width; i += (img_width<<1)) {
            for (j = 0; j < img_width; j += 2) {
                tmp =
                    ((bayer[i + img_width + j] + bayer[i + j + 1]) >> 1);
                CLIP16(tmp, outG[((i >> 2) + (j >> 1)) * 3], bits);
                tmp = bayer[i + img_width + j + 1];
                CLIP16(tmp, outR[((i >> 2) + (j >> 1)) * 3], bits);
                tmp = bayer[i + j];
...
dc1394error_t
dc1394_bayer_Raw_Ori_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    uint16_t *outR, *outG, *outB;
    register int i, j;
    int tmp;
    ...
    for (i = 0; i < img_height; i++) {
        for (j = 0; j < img_width; j++) {
            tmp = bayer[i * img_width + j];
            CLIP16(tmp, outG[(i * img_width + j) * 3], 16);
            CLIP16(tmp, outR[(i * img_width + j) * 3], 16);
            CLIP16(tmp, outB[(i * img_width + j) * 3], 16);
        }
    }
...
/* coriander's Bayer decoding */
dc1394error_t
dc1394_bayer_Simple_uint16(const uint16_t *restrict bayer, uint16_t *restrict rgb, int img_width, int img_height, int tile, int bits)
{
    uint16_t *outR, *outG, *outB;
    register int i, j;
    int tmp, base;

    // img_width and img_height should be even
    switch (tile) {
    case DC1394_COLOR_FILTER_GRBG:
    case DC1394_COLOR_FILTER_BGGR:
        outR = &rgb[0];
    ...
    case DC1394_COLOR_FILTER_GRBG:        //---------------------------------------------------------
    case DC1394_COLOR_FILTER_GBRG:
        for (i = 0; i < img_height - 1; i += 2) {
            for (j = 0; j < img_width - 1; j += 2) {
                base = i * img_width + j;
                tmp = ((bayer[base] + bayer[base + img_width + 1]) >> 1);
                CLIP16(tmp, outG[base * 3], bits);
                tmp = bayer[base + 1];
                CLIP16(tmp, outR[base * 3], bits);
                tmp = bayer[base + img_width];
                CLIP16(tmp, outB[base * 3], bits);
            }
        }
        for (i = 0; i < img_height - 1; i += 2) {
            for (j = 1; j < img_width - 1; j += 2) {
                base = i * img_width + j;
                tmp = ((bayer[base + 1] + bayer[base + img_width]) >> 1);
                CLIP16(tmp, outG[(base) * 3], bits);
                tmp = bayer[base];
                CLIP16(tmp, outR[(base) * 3], bits);
                tmp = bayer[base + 1 + img_width];
                CLIP16(tmp, outB[(base) * 3], bits);
            }
        }
        for (i = 1; i < img_height - 1; i += 2) {
            for (j = 0; j < img_width - 1; j += 2) {
                base = i * img_width + j;
                tmp = ((bayer[base + img_width] + bayer[base + 1]) >> 1);
                CLIP16(tmp, outG[base * 3], bits);
                tmp = bayer[base + img_width + 1];
                CLIP16(tmp, outR[base * 3], bits);
                tmp = bayer[base];
                CLIP16(tmp, outB[base * 3], bits);
            }
        ...
        for (i = 1; i < img_height - 1; i += 2) {
            for (j = 1; j < img_width - 1; j += 2) {
                base = i * img_width + j;
                tmp = ((bayer[base] + bayer[base + 1 + img_width]) >> 1);
                CLIP16(tmp, outG[(base) * 3], bits);
                tmp = bayer[base + img_width];
                CLIP16(tmp, outR[(base) * 3], bits);
                tmp = bayer[base + 1];
    ...
    /* add black border */
    for (i = img_width * (img_height - 1) * 3; i < img_width * img_height * 3; i++) {
        rgb[i] = 0;
    }
    for (i = (img_width - 1) * 3; i < img_width * img_height * 3; i += (img_width - 1) * 3) {
        rgb[i++] = 0;
        rgb[i++] = 0;
        rgb[i++] = 0;
    }
...
dc1394_bayer_VNG(const uint8_t *restrict bayer,
                 uint8_t *restrict dst, int img_width, int img_height,
                 dc1394color_filter_t pattern)
{
    ...
    dc1394_bayer_Bilinear(bayer, dst, img_width, img_height, pattern);
...
dc1394_bayer_VNG_uint16(const uint16_t *restrict bayer,
                        uint16_t *restrict dst, int img_width, int img_height,
                        dc1394color_filter_t pattern, int bits)
{
    ...
    dc1394_bayer_Bilinear_uint16(bayer, dst, img_width, img_height, pattern, bits);
...
dc1394error_t
dc1394_bayer_decoding_8bit(const uint8_t *restrict bayer, uint8_t *restrict rgb, uint32_t img_width, uint32_t img_height, dc1394color_filter_t tile, dc1394bayer_method_t method)
{
    switch (method) {
    case DC1394_BAYER_METHOD_NEAREST:
        return dc1394_bayer_NearestNeighbor(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_SIMPLE:
        return dc1394_bayer_Simple(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_BILINEAR:
        return dc1394_bayer_Bilinear(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_HQLINEAR:
        return dc1394_bayer_HQLinear(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_DOWNSAMPLE:
        return dc1394_bayer_Downsample(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_EDGESENSE:
        return dc1394_bayer_EdgeSense(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_VNG:
        return dc1394_bayer_VNG(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_AHD:
        return dc1394_bayer_AHD(bayer, rgb, img_width, img_height, tile);
    case DC1394_BAYER_METHOD_ORI:
        return dc1394_bayer_Raw_Origin(bayer, rgb, img_width, img_height, tile);
    default:
...
dc1394error_t
dc1394_bayer_decoding_16bit(const uint16_t *restrict bayer, uint16_t *restrict rgb, uint32_t img_width, uint32_t img_height, dc1394color_filter_t tile, dc1394bayer_method_t method, uint32_t bits)
{
    switch (method) {
    case DC1394_BAYER_METHOD_NEAREST:
        return dc1394_bayer_NearestNeighbor_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_SIMPLE:
        return dc1394_bayer_Simple_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_BILINEAR:
        return dc1394_bayer_Bilinear_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_HQLINEAR:
        return dc1394_bayer_HQLinear_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_DOWNSAMPLE:
        return dc1394_bayer_Downsample_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_EDGESENSE:
        return dc1394_bayer_EdgeSense_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_VNG:
        return dc1394_bayer_VNG_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_AHD:
        return dc1394_bayer_AHD_uint16(bayer, rgb, img_width, img_height, tile, bits);
    case DC1394_BAYER_METHOD_ORI:
        return dc1394_bayer_Raw_Ori_uint16(bayer, rgb, img_width, img_height, tile, bits);      
    default:
        return DC1394_INVALID_BAYER_METHOD;
    }
